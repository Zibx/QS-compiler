@info:
The `Header` component is inherited from the [Label](UI.Label) and behaves similarly to the `H1` tag in HTML markup. Can contain text or an [Image](UI.Image). The default position of `Header` is to the right of the preceding visual element.
