...
@info: The `DATA` object in QuokkaScript serves to access serialized data objects. As of the current version, only JSON objects are supported, XML objects support is still pending. `DATA` is handy when you need to parse a complicated data structure. You get a server response, for example, ` {name: John, surname: Doe, foo: 1337, bar: 65535}`. Then you place it into `DATA user: {{request.response}}` syntactic construction and further address to the received values in a short way: `Label: USERNAME: {{user.name}}`
